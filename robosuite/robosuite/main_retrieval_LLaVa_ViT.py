...
if __name__ == "__main__":
    ...
    for i in range(testing_iterations):
        ...
        obs = env.reset()
        env.close()       
        image = Image.fromarray(cv2.rotate(obs[f'frontview_image'], cv2.ROTATE_180))
        # save image as png 
        image.save(f'code_results/box/LLaVA_ViT/image.png')
        image = 'code_results/box/LLaVA_ViT/image.png'
        # image = '/home/xilun/ET_robot/robosuite/robosuite/render/dummyDemo_image.png'
        temperature = 0.05

        completion = LLaVA_call_with_retry(model_path, prompt_text, image, temperature, max_tokens=1000)
            
        ## only save the python code part from the completion
        completion_code = completion.split('```python')[1].split('```')[0]
        with open(f'code_results/box/LLaVA_ViT/python_output_{i}.txt', 'w') as f:
